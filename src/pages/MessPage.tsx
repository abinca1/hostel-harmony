...
const MessPage = () => {
  ...
  const sortedMeals = [...menuItems]
    ...
    return mealTypeOptions.indexOf(a.mealType) - mealTypeOptions.indexOf(b.mealType);
  });
  ...
  const handleSaveMeal = () => {
    ...
    setMenuItems(prev => {
      ...
      if (exists) {
        return prev.map(item => (item.id === mealDraft.id ? mealDraft : item));
      }
  ...
  const parseDietaryTags = (value: string): DietaryTag[] =>
    value
      ...
      .filter(tag => dietaryTagOptions.includes(tag as DietaryTag)) as DietaryTag[];
  ...
  return (
    <div className="animate-fade-in">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {meal.dietaryTags.map(tag => (
                                  <Badge
                                    key={tag}
                                    variant="outline"
                                    className={cn('text-xs capitalize', dietaryColors[tag])}
                                  >
                                    {tag}
                                  </Badge>
                                ))}
      ...
      <Dialog open={isMealDialogOpen} onOpenChange={setIsMealDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{mealDraft?.name ? 'Edit Meal' : 'Add Meal'}</DialogTitle>
          </DialogHeader>
          {mealDraft ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={value =>
                      setMealDraft({ ...mealDraft, mealType: value as MealType })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
              ...
              <div className="space-y-2">
                <Label htmlFor="meal-tags">Dietary Tags (comma separated)</Label>
                <Input
                  id="meal-tags"
